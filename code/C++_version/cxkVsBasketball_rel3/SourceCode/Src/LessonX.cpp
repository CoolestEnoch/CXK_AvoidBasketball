...
#include <cctype>					//atoi(); atof();
using namespace std;

//默认设置量
const float chickenSpeedBase = -40;
float chickenSpeed = chickenSpeedBase;
const float basketballSpeedBase = 40;
float basketballSpeed = basketballSpeedBase;
const float cxkSpeedBase = 30;
float cxkSpeed = cxkSpeedBase;
...
void startGame(){
	...
	// 初始化配置数据
	chickenSpeed = chickenSpeedBase;
	basketballSpeed = basketballSpeedBase;
	...
	PlaySound(sounds[randNum(1)].data(),NULL,SND_FILENAME|SND_ASYNC|SND_LOOP);
...
void stopGame(){
	...
	PlaySound("game/data/audio/ngm.wav",NULL,SND_FILENAME|SND_ASYNC);
...
{	
	...
	case KEY_W:
	case KEY_UP:
		cxk->SetSpriteLinearVelocityY(cxkSpeed * -1.0);
		break;
	case KEY_A:
	case KEY_LEFT:
		cxk->SetSpriteLinearVelocityX(cxkSpeed * -1.0);
		break;
	case KEY_S:
	case KEY_DOWN:
		cxk->SetSpriteLinearVelocityY(cxkSpeed);
		break;
	case KEY_D:
	case KEY_RIGHT:
		cxk->SetSpriteLinearVelocityX(cxkSpeed);
		break;
	case KEY_LCONTROL:
	case KEY_RCONTROL:
	case KEY_CONTROL:
...
void CGameMain::OnSpriteColSprite( const char *szSrcName, const char *szTarName )
{
	if(running){
		// 鸡受到了碰撞
		if(string(szTarName).find("chicken") == 0){
			int chickenIndex = atoi(&szTarName[7]);

			// 撞上蔡徐坤
			if(strcmp(szSrcName, "cxk") == 0){
				stopGame();
			}
			
			// 撞上篮球
			if(strcmp(szSrcName, "basketball") == 0){
				if(chickensPtr[chickenIndex]->GetSpriteLinearVelocityX() < 0){
					chickensPtr[chickenIndex]->SetSpriteLinearVelocityX(chickensPtr[chickenIndex]->GetSpriteLinearVelocityX() * -1.0);
					score++;
				}
			}

			// 撞上菜碧
			if(strcmp(szSrcName, "caibi") == 0){
				if(chickensPtr[chickenIndex]->GetSpriteLinearVelocityX() < 0){
					chickensPtr[chickenIndex]->SetSpriteLinearVelocityX(chickensPtr[chickenIndex]->GetSpriteLinearVelocityX() * -1.0);
				}
			}
		...
		// 篮球撞上鸡
		//if(string(szTarName).find("chicken") == 0 && strcmp(szSrcName, "caibi") != 0){
		//	int index = szTarName[7];
		//	if(chickensPtr[index]->GetSpriteLinearVelocityX() < 0)
		//		score++;
		//}

		// 动态调整游戏参数
		// 动态调整鸡的速度
		chickenSpeed = chickenSpeedBase - score * 0.5;// 鸡的速度是个负值
		// 动态调整篮球速度
		basketballSpeed = basketballSpeedBase + score * 0.4;// 球速度是正值
		// 动态调整蔡徐坤速度
		cxkSpeed = cxkSpeedBase + score * 0.5;
		// 让没出发的鸡出发
		int index = randNum(chicken_num - 1);
		if(chickensPtr[index]->GetSpriteLinearVelocityX() == 0)
			chickenReGo(index);
